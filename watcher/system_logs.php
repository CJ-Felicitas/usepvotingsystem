...
<?php
    ...
    $mUser = str_replace("watcher","admin",$user);
    $conn = new mysqli(DB_HOST,DB_USER,DB_PASS,DB_NAME);
    $query = "DELETE from tblogs where name = '$mUser' ";
...
<?php
  ...
  $mTempUser = str_replace("watcher","admin",$temp);
  $conn = new mysqli(DB_HOST,DB_USER,DB_PASS,DB_NAME);
  $query  = "SELECT * FROM tblogs where name = '$mTempUser' ORDER BY timestamp DESC";
  $nominees = array();
  $result = mysqli_query($conn,$query);
  while ($row = mysqli_fetch_assoc($result)) {
    $logs[] = $row;
...
<html lang="en">  
  ...
  <body id="page-top">
    ...
    <a id="title" class="navbar-brand mr-1" href="../watcher/home.php"> USeP E-Voting</a>
    ...
    <div id="wrapper">
      ...
      <li class="text-center">
          ...
          <li class="nav-item">
          <a class="nav-link" href="../watcher/home.php">
            <i class="fas fa-fw fa-tachometer-alt"></i>
            <span>Dashboard</span>
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link" href="../watcher/election/result.php">
            <i class="fas fa-fw fa-list-alt"></i>
            <span>Election Result</span></a>
        </li>
        <li class="nav-item active bg-danger">
          <a class="nav-link" href="../watcher/system_logs.php">
      ...
      <div id="content-wrapper">
        ...
        <?php foreach($logs as $log) :  ?>
            <tr>
            <td>
                <?= date('g:i a | m-d-Y', strtotime($log['timestamp'])); ?>
            </td>
                <td><?= $log['name'] ?></td>
                <td><?= $log['action']; ?></td>
                </tr>
