<?php
    ...
    //Student Choose SSG For The Mode Of Voting
    if ($mode == 'University Student Government'){
        //Check For Proxy Voting In SSG
        $stmtProxy= $pdo->prepare('SELECT * FROM voter_pub_ip_ssg WHERE public_ip_address= :ip ');
        $stmtProxy->execute([ 'ip' => $final ]);
        if($stmtProxy ->rowCount() > 0){
            $proxy = 'true';
        }
        //Check For Election Date In SSG
        $stmt= $pdo->prepare('SELECT * from tbelectiondate_other WHERE indicator = :indicator');
        $stmt->execute(['indicator' => "SSG"]);
        if($stmt ->rowCount() > 0){
        ...
        if($currDT > $electionEnd){
            echo json_encode(array('Election Due','University Student Government')); 
        }else{
            //Check For If they are voters In SSG
            $stmtVoter= $pdo->prepare('SELECT * FROM tbssgvoters WHERE campus= :campus AND stud_id= :username AND password= :password');
            $stmtVoter->execute([ 'campus' => $campus , 'username' => $username , 'password' => $password]);
            if($stmtVoter ->rowCount() > 0){
                while($row = $stmtVoter->fetch()){
                    //Check For If they have voted In SSG
                    $stmtVoted= $pdo->prepare('SELECT * FROM tb_ssgvotes WHERE campus= :campus AND studID= :username ');
                    ...
                    if($stmtVoted ->rowCount() > 0){
                        ...
                        if($proxy == 'true'){
                            ...
                            //Check For If they have already login with another device In SSG
                            $stmtLogs= $pdo->prepare('SELECT * FROM tbssgloginlogs WHERE stud_id= :username AND campus= :campus');
                                    ...
                                    $message = "Your Code:'".$otpsend."'";
                                        ...
                                        $mail->setFrom('comelecusep@gmail.com','USeP E-Voting')
                                            ...
                                            $mailer = new Nette\Mail\SmtpMailer([
                                            	'host' => 'smtp.gmail.com',
                                            	'username' => 'comelecusep@gmail.com',
                                            	'password' => 'xbszyvjesmuufzcf',
                                            	'secure' => 'ssl',
                                            ]);
                                            $mailer->send($mail);
                                        
                                        mail("'.$email.'","'.$subject.'","'.$message.'","From: DoNotReply@gmail.com");
                                        ...
                                        setcookie("Uemail", $email, time() + (60 * 15)); //set email cookie attached with 15 minutes
        ...
        //Check For Proxy Voting In Plebiscite
        $stmtProxy= $pdo->prepare('SELECT * FROM voter_pub_ip_pleb WHERE public_ip_address= :ip ');
        $stmtProxy->execute([ 'ip' => $final ]);
        if($stmtProxy ->rowCount() > 0){
            $proxy = 'true';
        }
        ...
        $stmt= $pdo->prepare('SELECT * from tbelectiondate_other WHERE indicator = :indicator');
        $stmt->execute(['indicator' => "Plebiscite"]);
        if($stmt ->rowCount() > 0){
            while($row = $stmt->fetch()){
                $electionEnd = $row['end_date']." ".$row['end_time'];
            }
        ...
        if($currDT > $electionEnd){
            echo json_encode(array('Election Due','Plebiscite')); 
        }else{
            //Check For If they are voters In Plebiscite
            $stmtVoter= $pdo->prepare('SELECT * FROM tbvoters WHERE campus= :campus AND stud_id= :username AND password= :password');
            $stmtVoter->execute([ 'campus' => $campus , 'username' => $username , 'password' => $password]);
            if($stmtVoter ->rowCount() > 0){
                while($row = $stmtVoter->fetch()){
                    //Check For If they have voted In Plebiscite
                    $stmtVoted= $pdo->prepare('SELECT * FROM tbplebvotes WHERE campus= :campus AND studID= :username ');
                    $stmtVoted->execute([ 'campus' => $campus , 'username' => $username ]);
                    if($stmtVoted ->rowCount() > 0){
                            echo json_encode(array('Voted')); 
                    }else{
                        if($proxy == 'true'){
                            ...
                            //Check For If they have already login with another device In Plebiscite
                            $stmtLogs= $pdo->prepare('SELECT * FROM tbplebloginlogs WHERE stud_id= :username AND campus= :campus');
                            $stmtLogs->execute([ 'username' => $username , 'campus' => $campus ]);
                            if($stmtLogs ->rowCount() > 0){
                                echo json_encode(array('Login')); 
                            }else{
        ...
        //Student Choose Obrero, Tagum, Mintal and Mabini For The Mode Of Voting

        //Check For Proxy Voting In Campus Election
        $stmtProxy= $pdo->prepare('SELECT * FROM voter_pub_ip WHERE public_ip_address= :ip ');
        $stmtProxy->execute([ 'ip' => $final ]);
        if($stmtProxy ->rowCount() > 0){
            $proxy = 'true';
        }
